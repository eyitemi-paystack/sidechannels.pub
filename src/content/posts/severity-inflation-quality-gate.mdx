...
What I found was a report that did not match the bug.

I had called the finding "RCE" when it was localhost-only. I had called it "zero-click" when the attack required a specific application state. I had stacked four CWE identifiers to make the finding look more severe. I had written a 1,200-word scope argument because the bug sat near a program boundary I knew was contested. I had opened with the sentence *"no claims in this report are speculative"*—and several were. Every one of these moves was something I would have flagged in someone else's report. In my own, I did not see them until the bug was no longer the first thing in my head.
...
If the rejection of one report were a sample size of one, it would not be worth a post. But the patterns in my own corrections corpus map almost exactly onto patterns visible in public—at every scale of program, in every corner of the disclosure ecosystem.
...
This is not only an Anthropic story. [Google Project Zero published "From Naptime to Big Sleep"](https://googleprojectzero.blogspot.com/2024/10/from-naptime-to-big-sleep.html) in November 2024—the first widely-cited demonstration of LLM-driven vulnerability discovery in production code. [Sean Heelan used OpenAI's o3 to find CVE-2025-37899](https://sean.heelan.io/2025/05/22/how-i-used-o3-to-find-cve-2025-37899-a-remote-zeroday-vulnerability-in-the-linux-kernels-smb-implementation/)—a real Linux kernel SMB zero-day—in May 2025. [Joshua Rogers documented 49 completed AI-assisted curl PRs](https://joshua.hu/llm-engineer-review-sast-security-ai-tools-pentesters) in October 2025, on the maintainer side rather than the lab side. The Frontier Red Team arc that follows sits inside a broader story of which Anthropic is one node—but the most legible single arc to walk through is the one Anthropic published in chronological order, so that is the one I work with here.
...
- **[August 2025](https://red.anthropic.com/2025/cyber-competitions/):** Claude finished top 3% in PicoCTF (297th of 10,460 teams)
- **[September 2025](https://red.anthropic.com/2025/ai-for-cyber-defenders/):** The team named the moment: *"an inflection point for AI's impact on cybersecurity"*
...
- **[January 2026](https://red.anthropic.com/2026/cyber-toolkits-update/):** Autonomous success on Equifax cyber range—no custom toolkit required
...
- **[March 2026](https://red.anthropic.com/2026/firefox/):** 22 Firefox vulns in two weeks, 14 high-severity (~19% of all 2025 high-sev Firefox CVEs), \$4,000 in API credits
...
- **Firefox exploits:** Mythos achieved **181 successful exploits**, against Opus 4.6's *"two times out of several hundred attempts"*
- **[OSS-Fuzz](https://google.github.io/oss-fuzz/):** **595 crashes at tiers 1 and 2**, plus *"ten separate, fully patched targets (tier 5)"*
- **OpenBSD:** *"A thousand runs through our scaffold… under \$20,000"*
- **FreeBSD:** an exploit for a 17-year-old NFS RCE (CVE-2026-4747) completed *"after several hours"* at under \$50 per individual run
- **FFmpeg:** several hundred runs over the repository for roughly \$10,000
- **Validator agreement:** **89% of 198 manually reviewed vulnerability reports** matched Claude's severity assessment exactly
- **Historical bugs found:** a 27-year-old OpenBSD SACK bug (DoS-grade crash of any OpenBSD host), a 16-year-old FFmpeg H.264 vulnerability, the FreeBSD NFS RCE above (unauthenticated remote root), a guest-to-host VMM vulnerability *despite Rust*, and Linux kernel privilege escalations chaining [KASLR](https://en.wikipedia.org/wiki/Address_space_layout_randomization) bypasses with use-after-frees
...
Read the two announcements together and the structural picture is impossible to miss. **A frontier lab that did not set out to build a vulnerability research engine has built one anyway, by accident, as a downstream effect of scaling. The same lab is responding by trying to give the engine to defenders before it leaks to attackers.** Three of the partner CSOs:
...
**Production side:**
- Smart contract exploits: \$1.22 per agent run, exploit revenue doubling every 1.3 months
- Firefox: \$4,000 in API credits → 22 vulnerabilities, 14 high-severity, ~19% of all 2025 high-sev Firefox CVEs
- OpenBSD: \$20,000 across 1,000 scaffold runs
- FreeBSD: under \$50 for a single run that yielded a 17-year-old unauthenticated remote root
- FFmpeg: ~\$10,000 across several hundred runs
...
What follows is one specific structural answer—mine. Not the right one, not the only one; *a* worked example of the validation infrastructure the current environment requires.
...
[§7](#7-limits-and-failure-modes) follows with the limits of this approach—including the cases where the validation engine I have just described fails on its own terms.

---

## 7. Limits and failure modes

The Quality Gate is a debiasing instrument built by one researcher against the failure modes of one researcher's prior reports. Its limits are real and worth naming explicitly. I prefer to name them here rather than have a triager name them for me.
...
Section 6.5 argues that the same model that produces inflated reports can be structured adversarially to validate them. That argument is true on average. It is not always true. The post-research-audit skill has its own failure modes, which I have observed in my own use:
...
*— Eyitemi Egbejule ([@eeyitemi](https://x.com/eeyitemi)), April 8, 2026*
